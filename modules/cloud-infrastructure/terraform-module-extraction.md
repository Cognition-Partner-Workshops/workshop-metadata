...
> Analyze the Terraform configurations in timesheet-infra and refactor them into reusable modules. Extract at least 3 modules: networking (VPC, subnets, security groups), compute (EC2 instance, IAM roles), and container registry (ECR repository, lifecycle policies). Each module should have clear input variables, outputs, and a README with usage examples. Refactor the root module to use the extracted child modules. Ensure `terraform plan` shows no changes after refactoring. Open a PR with the modularized Terraform, module documentation, and a migration guide.
