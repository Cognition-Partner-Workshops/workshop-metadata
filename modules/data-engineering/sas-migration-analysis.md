...
The target migration path is **SAS → dbt on Databricks**, using the dbt project structure in `uc-data-migration-sas-to-snowflake/dbt_project/` as the reference target architecture.
...
| Format Definition | `banking_formats.sas` | Custom PROC FORMAT value/informat definitions | Data decoding rules → dbt macros or seed tables |
| Batch Orchestrator | `run_daily_*.sas` | %INCLUDE chains, error handling, Control-M integration | Job dependency graph → Databricks Workflows |
| SAS Dataset | `.sas7bdat` | Binary data files | Schema extraction, sample data for validation |

## Target Outcomes

- Complete inventory of all SAS artifacts: programs (7 business programs + 92 macros), Enterprise Guide projects (2 .egp), deployment packages (1 .spk), format definitions, batch orchestrators, and log files
- Data lineage documentation tracing dataset flows from Oracle/Teradata sources through DATA steps, PROC SQL, and macro invocations to curated/reporting outputs
- Macro dependency graph showing which programs depend on which macros, and which macros call other macros
- Dataset usage matrix: which programs read, create, update, or delete each dataset (including cross-library references from autoexec.sas LIBNAME assignments)
- SAS construct → dbt/Databricks mapping using `docs/SAS_TO_DBT_MIGRATION_MAP.md` as the reference
- Complexity assessment scoring each program by number of DATA steps, PROC operations, macro calls, external database dependencies, and migration difficulty
...
SAS-to-Databricks migration toolkit with sample banking datasets (.sas7bdat + CSV), Collibra-style lineage metadata (SAS_lineage.json, SF_lineage.json), a complete dbt target project with staging/intermediate/marts layers, dbt macros replacing PROC FORMAT definitions, a comprehensive SAS→dbt construct mapping document, validation configurations, and a Streamlit migration app. The dbt project maps 1:1 from the SAS programs in ts-sas-legacy-analytics.

### Step 1: Paste into Devin — Lineage and Validation

> Analyze the lineage metadata in uc-data-migration-sas-to-snowflake/lineage/ (SAS_lineage.json and SF_lineage.json). Map the lineage nodes and edges to the data flows identified in the SAS migration assessment. Then review the dbt project in dbt_project/ — verify that each dbt model correctly implements the SAS logic from ts-sas-legacy-analytics. Cross-reference the migration patterns in docs/SAS_TO_DBT_MIGRATION_MAP.md. For any gaps (SAS programs without corresponding dbt models), create stub dbt models following the established pattern. Validate using the rules in config/validation_rule_config.json. Open a PR.

### Step 2: Paste into Devin — Extend dbt Models

> The dbt_project/ in uc-data-migration-sas-to-snowflake has models for customer accounts, transactions, and risk scoring. Add the missing dbt models for: (1) monthly_regulatory_reporting.sas → mart_regulatory_rwa.sql + mart_delinquency_aging.sql, (2) claims_processing.sas → stg_claims.sql + int_claims_adjudication.sql, (3) policy_valuation.sas → int_policy_valuation.sql + mart_loss_ratios.sql, (4) customer_profitability.sas → mart_customer_pnl.sql. Follow the patterns in the existing models and reference the SAS_TO_DBT_MIGRATION_MAP.md for construct translations. Add dbt tests for key business rules. Open a PR.

### Step 3 (Optional): Read the DeepWiki

Open the DeepWiki pages for both ts-sas-legacy-analytics and uc-data-migration-sas-to-snowflake to understand the full migration pipeline from SAS source through assessment to dbt target.
...
- **Validation-driven migration**: The Streamlit app and validation configs provide a QA framework for verifying migration correctness
- **Lineage preservation**: Collibra-style JSON lineage bridges the gap between SAS-side and Databricks-side data flows
